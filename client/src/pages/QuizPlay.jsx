...
const QuizPlay = () => {
  ...
  const triggerComplete = useCallback(async () => {
    ...
    setCompleting(true);
    try {
      await quizAPI.complete({ sessionId: paramId, timeSpentTotal: elapsedRef.current });
  ...
  const handleSelect = (idx) => {
    ...
    if (submittedAnswersRef.current[currentIndex] !== idx) {
      ...
      quizAPI
        .answer({ sessionId: paramId, questionIndex: currentIndex, answerIndex: idx, timeSpent: 0 })
      ...
      <div className="qp-page">
        ...
        <div className="qp-review-sheet">
              ...
              return (
                ...
                >
                  <div className="qp-review-num">
                    <span className={`qp-review-badge ${ans !== null ? 'qp-review-badge--answered' : 'qp-review-badge--blank'}`}>
                      Q{i + 1}
                    </span>
                  </div>
                  <div className="qp-review-body">
                    ...
                    <div className="qp-review-ans">
                      {ans !== null ? (
                        ...
                        <span className="qp-review-ans--none">Not answered</span>
                      )}
                    </div>
                  </div>
                  {!autoSubmitRef.current && (
          ...
          <div className="qp-review-footer">
            {!autoSubmitRef.current && (
              <button className="qp-review-back-btn" onClick={() => setShowReview(false)}>
                ← Back to Quiz
              </button>
            )}
            <button
      ...
      <div className="qp-layout">
        ...
        <div className="qp-main">
          ...
          <div className="qp-nav">
            ...
            <span className={`qp-nav-status ${localAnswers[currentIndex] !== null ? 'qp-nav-status--answered' : ''}`}>
              {localAnswers[currentIndex] !== null ? 'Answered' : 'Not answered'}
            </span>

            <button className="qp-nav-btn qp-nav-btn--next" onClick={handleNext}>
        ...
        <div className="qp-sidebar">
          ...
          {/* Timer card */}
          <div className={`qp-timer-card${timerCritical ? ' qp-timer--critical' : timerWarning ? ' qp-timer--warning' : ''}`}>
            ...
            {timerWarning && !timerCritical && (
              <div className="qp-timer-msg qp-timer-msg--warn">Less than 2 minutes!</div>
            )}
            {timerCritical && (
          ...
          <div className="qp-tracker-card">
            ...
            <div className="qp-tracker-legend">
              <span className="qp-leg"><span className="qp-leg-dot qp-leg-dot--answered" />Answered</span>
              <span className="qp-leg"><span className="qp-leg-dot qp-leg-dot--skipped" />Skipped</span>
              <span className="qp-leg"><span className="qp-leg-dot qp-leg-dot--current" />Current</span>
            </div>

            <div className="qp-tracker-count">{answeredCount} / {questions.length} answered</div>

            <button className="qp-review-trigger" onClick={() => setShowReview(true)}>
              Review &amp; Submit
            </button>
          </div>
